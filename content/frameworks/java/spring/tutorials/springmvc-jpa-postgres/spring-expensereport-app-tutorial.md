...
2.  Have completed the previous exercise.
...
Once a Normal User submits an expense, it goes to the Admin's work items. The Admin can then Approve or Reject the expense.
...
map.put(org.hibernate.cfg.Environment.HBM2DDL_IMPORT_FILES, "import.sql");
```
* Create a new file `config.properties` in `src/main/resources` and add the following database properties in order to connect with your local PostgreSQL server.
