...
## Introduction
In this tutorial, you will create Expense Reporting App using Spring MVC and PostgreSQL database and deploy it on Cloud Foundry.
...
## Application Flow:
* The Fig 2.3 shows the runtime interaction of the application.

	![sequence-flow](/images/spring_tutorial/sequence-flow.png)

     Fig 2.3

1.  User login to the Expense Reporting system.
...
In order to work with data from a PostgreSQL database, we need to obtain a connection to the database for that define dataSource bean. If your PostgreSQL serevr requires authentication then set username and password of your PostgreSQL server in dataSource bean otherwise don't set these two properties. We have to set `org.hibernate.cfg.Environment.HBM2DDL_AUTO` JPA property, to create tables automatically for our Entities.
```java
@Configuration
...
map.put(org.hibernate.cfg.Environment.HBM2DDL_IMPORT_FILES, "import.sql");
```
* Create a new file `config.properties` in `src/main/resources` and add the following database properties to connect with local PostgreSQL server.
...
## **Step 6:** Styles & JavaScript Files
You can download the Expense Reporting App styles and javascript code [here](/code/tutorials/springmvc-jpa-postgres/expensereport/resources.zip)
