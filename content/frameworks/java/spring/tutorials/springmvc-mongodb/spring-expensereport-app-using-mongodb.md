...
MongoDB is a document oriented database, the java classes will be stored as BSON documents. The domain classes are marked with @Document annotation to indicate that this class is a candidate to map to the database. By default, the class name is chosen as the collection name. If you want to store it under a different name, set under collection property.
```java
...
```
 Now implement `MongoDbFactoryConfiguration` for Cloud Foundry dataSource as `CloudMongoDbFactoryConfiguration` and use `@Profile("cloud")` annotation. To use the Cloud Foundry API add the following dependency has been added in `pom.xml`.
```xml
<dependency>
...
Modify the `db.properties` for hostname, port and database name according to your local MongoDB database.
