...
As explained in [the overview](/frameworks/ruby/rails-tutorial/rails-new-template-with-postgres#expense-reporting-application-overview), the application contains two roles. The application operates based on the user role.

We have two roles, admin and normal user. Normal Users should be able to create their own expenses and be to edit or delete them. An Admin can accept or reject expenses. `CanCan` is an authorization library for Ruby on Rails which restricts the resources a given user is allowed to access.
...
`normal user` has been given as the default value for this field, so every user who signs up from the browser will be a normal user. An admin user cab be created by console. 
...
The above method will return `true` if the user is admin and false if not. This method is useful in handling specific actions to check whether a user ha admin access or not.
...
Finally, the migration file for expense creation will be like follows.
...
Run the command for creating the tables and fields.
...
Now its time to create the controller and view part for the expense creation. Expense can be created by a normal user only.
...
Add the content for `/app/views/expenses/edit.html.erb` file as given
...
The`/app/views/expenses/index.html.erb` file looks like so.
...
Now open your browser and login as a user. You will be redirected to your expenses index page.


[![Prev](/images/prev.png)](/frameworks/ruby/rails-tutorial/rails-user-login.html)  <span style="float: right;">[![Next](/images/next.png)](/frameworks/ruby/rails-tutorial/rails-expense-admin-flow.html)</span>
