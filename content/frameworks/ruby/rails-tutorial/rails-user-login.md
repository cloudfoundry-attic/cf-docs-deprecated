...
Below is the tutorial to wrote the template for `new` action. It will have a form consisting of `email, password and password_confirmation` fields along with some code for showing any validation errors. Hence the following code should go into `/app/views/users/new.html.erb`.
...
Things get more interesting in the SessionsController. We have a new action here, but we don’t need to add any code to it so we’ll move on to the template. Inside the new view file that was generated we shall create the form for signing in. Hence the following code should go into /app/views/sessions/new.html.erb
...
and below is the code for the partial `_header_nav.html.erb` which mentioned above in the `application.html.erb` layout
